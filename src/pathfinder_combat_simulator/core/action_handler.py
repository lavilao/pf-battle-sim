import random
from typing import Dict, List, Optional, Any, Tuple # Added for full context from original file

# Assuming these are the correct relative imports for core components
from .combat_engine import CombatEngine
from .combatant import Combatant
from .enums import ActionType, AttackType
from .attack import Attack # Added Attack
from .attack_result import AttackResult
from .ability_scores import AbilityScores # Added AbilityScores, might be needed by e.g. get_attack_bonus indirectly
...
class ActionHandler:
    ...
    def can_take_action(self, combatant: Combatant, action_type: ActionType) -> bool:
        ...
        if combatant.has_condition("stunned") or \
           ...
           combatant.is_dying(): # Dying combatants are unconscious
            ...
            conditions_list = [c for c in ['stunned', 'paralyzed', 'helpless', 'unconscious', 'dead', 'dying'] if (c=='dead' and combatant.is_dead()) or (c=='dying' and combatant.is_dying()) or (combatant.has_condition(c) and c not in ['dead', 'dying']) ]
            if conditions_list:
                self.combat_engine.log.add_entry(f"{combatant.name} cannot take actions due to condition ({conditions_list[0]}).")
            return False
    ...
    def take_attack_action(self, attacker: Combatant, target: Combatant,
        ...
        if not attacker.attacks or attack_index >= len(attacker.attacks): # Check if list is empty
    ...
    def take_full_attack_action(self, attacker: Combatant, target: Combatant,
                               attack_index: int = 0) -> List[AttackResult]: # Added default for attack_index
        ...
        results = []
        attack = attacker.attacks[attack_index] # This uses the attack_index for the *primary* sequence
        ...
        for i in range(num_attacks):
            ...
            if current_iterative_bab < 0 and i > 0 : # Only allow negative BAB if it's the first attack and BAB is negative
                 break
    ...
    def take_cast_spell_action(self, caster: Combatant, spell_name: str, target: Optional[Combatant] = None): # spell_level removed as per original
    ...
    def take_aid_another_action(self, actor: Combatant, target_creature_to_hinder: Combatant, ally_to_aid: Combatant, aid_type: str = "attack") -> bool:
        ...
        if actor.attacks:
            for attack_obj in actor.attacks: # renamed attack to attack_obj
                if attack_obj.attack_type == AttackType.MELEE or attack_obj.attack_type == AttackType.NATURAL:
    ...
    def take_move_action(self, combatant: Combatant, distance: int = 0, provokes_aoo: bool = True): # Added typehint for distance
        if not self.can_take_action(combatant, ActionType.MOVE):
        ...
        if distance <= max_distance: # Distance can be 0 for taking a move action to do something else
    ...
    def take_charge_action(self, attacker: Combatant, target: Combatant, charge_attack_index: int = 0) -> Optional[AttackResult]:
        ...
        if not attacker.attacks or charge_attack_index >= len(attacker.attacks) or attacker.attacks[charge_attack_index].attack_type == AttackType.RANGED:
        ...
        attack_result = self.combat_engine.make_attack(attacker, target, charge_attack,
                                                       is_full_attack=False, attack_number=0,
    ...
    def take_bull_rush_action(self, attacker: Combatant, target: Combatant, as_part_of_charge: bool = False) -> bool:
        ...
        if as_part_of_charge: self.combat_engine.log.add_entry(f"  Gains +2 bonus from charging.")
        ...
        if not maneuver_result: return False
        ...
        if total_maneuver_roll >= target_cmd or d20_roll == 20:
            ...
            push_distance = 5 + ((total_maneuver_roll - target_cmd) // 5) * 5 if (total_maneuver_roll - target_cmd) >=5 else 5
    ...
    def take_disarm_action(self, attacker: Combatant, target: Combatant) -> bool:
        ...
        if is_unarmed_attempt: self.combat_engine.log.add_entry(f"  Attacker is unarmed, takes -4 penalty on Disarm attempt.")
        ...
        if total_maneuver_roll >= target_cmd or d20_roll == 20:
            ...
            self.combat_engine.log.add_entry(f"  {target.name} drops {items_dropped} item{'s' if items_dropped > 1 else ''} (item system not implemented).")
            if is_unarmed_attempt and items_dropped > 0: self.combat_engine.log.add_entry(f"  {attacker.name} (unarmed) may automatically pick up one dropped item (not simulated).")
            return True
        else:
